...
# Load the dataset
X, y = load_dataset(400, n_clusters)
...
def calculate_pg_stats(dataframe):
    """
    This function calculates the mean Population growth for a specific year
    :param dataframe: Dataframe containing the Population growth data
    :return: None
    """
    # Calculate the mean Population growth (% annual) for each country
    mean_pg = dataframe.mean()
    print("Mean PG : ")
    print(mean_pg)
    print("\n\n")

    # Calculate the  Population growth (% annual) for each country in 2013
    pg_2013 = dataframe[dataframe['Year'] == 2013]
    print("PG 2013 : ")
    print(pg_2013)

calculate_pg_stats(df2)

def plot_pg_over_time(df, countries):
    """
    This function plots the Population growth (% annual) for a given list of countries over time
    :param df: dataframe containing Population growth data
    :param countries: list of countries to plot Population growth for
    """
    df.plot(x='Year', y=countries, color=['#6dd47e','#ffd55a','#293250'])
    plt.title("Population growth for each country", fontsize=15)
...
plot_pg_over_time(df2, country_name)
...
def plot_population_growth(df, country):
    """
    This function plots the Population growth of a given country over time
    :param df: DataFrame containing the Population growth data
    :param country: name of the country for which the Population growth should be plotted
    """
    df.plot("Year", country, color="blue")
    plt.title(f"{country}'s Population Growth (% annual)", fontsize=15)
    plt.xlabel("Year", fontsize=14)
    plt.ylabel("% annual", fontsize=14)
    plt.legend(["PG"])
    plt.show()

plot_population_growth(transposed_df, country[0])
...
def plot_logistic_fit(df, country):
    """
    Plot a logistic fit of the given country's Population growth
    :param df: DataFrame containing the PG data
    :param country: string of the country to plot
    """
    df.plot("Year", [country, "fit"], color=["blue","red"])
    plt.title("Logistic fit of {}'s Population growth".format(country), fontsize=15)
    plt.xlabel("Year", fontsize=14)
    plt.ylabel("% annual", fontsize=14)
    plt.legend(["pg"])
    plt.show()
...
def plot_future_prediction(df, country, forecast, year):
    """
    Plot the future prediction of a country's Population growth using a logistic model
    :param df: DataFrame containing the PG data
    :param country: string of the country to plot
    :param forecast: forecasted values for the future
    :param year: future year
    """
    plt.plot(df["Year"], df[country], label="PG", color="blue")
    plt.plot(year, forecast, label="forecast", color="red")

    plt.title("Future Year prediction using Logistic of {}'s Population growth".format(country), fontsize=15)
    plt.xlabel("Year", fontsize=14)
    plt.ylabel("% annual", fontsize=14)
    plt.legend(["PG"])
    plt.legend()
...
def plot_prediction_limits(df, country, forecast, year, low, up):
    """
    Plot the future prediction of a country's Population growth using a logistic model along with lower and upper limits
    :param df: DataFrame containing the PG data
    :param country: string of the country to plot
    :param forecast: forecasted values for the future
    :param year: future year
    ...
    """
    plt.figure()
    plt.plot(df["Year"], df[country], label="EPC", color="blue")
    plt.plot(year, forecast, label="forecast", color="red")

    plt.fill_between(year, low, up, color="orange", alpha=0.7)
    plt.title("Upper and Lower Limit of {}'s Population growth".format(country), fontsize=15)
    plt.xlabel("Year", fontsize=14)
    plt.ylabel("% annual", fontsize=14)
    plt.legend()
    plt.show()
